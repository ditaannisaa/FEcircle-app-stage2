...
export default function Foryou() {
  ...
  // const users = allUser?.filter(
  //   (item: any, index: number) => item.id != user?.user?.id && index < 4
  // );
  // console.log(users, "ini users ya");
  ...
  const isFollowing = (userId: number | undefined) => {
    return (
      user.user?.following?.some((follow: TFollow) => follow.id === userId) ??
      false
    );
  };
